...
import com.example.sd1000application.R.color;
...
public class TestUpActivity extends Activity{
	...
	private Handler mHandler=new Handler(){
	    public void handleMessage(Message msg){
			...
			if(num==60)
			{
				float[] rect=GetRectOut(60);
				canvas.drawRect(rect[0], rect[1], rect[2], rect[3], labbackPaint);
				canvas.drawText(Integer.toString(MidData.LaveTestTime), 30+5*xSpace, 50+5*ySpace, textLabPaint);
			...
			if(num!=LastTouchNum)
			{
				...
				//System.out.println(addressNum+"---ASAS----"+status+"---"+rect[0]+"---"+rect[1]+"---"+rect[2]+"---"+rect[3]);
				switch (status) {
				case 1:
	...
	private float[] GetRectOutLabS(int num)
	{		
		float[] rect=new float[4];
		int x=num%11;
		int y=num/11;
		...
		rect[1]=(float) (y*ySpace+1);
		rect[2]=(float) ((x+1)*xSpace-1);
		rect[3]=(float) ((y+1)*ySpace-1.5);
		return rect;
	}
	private float[] GetRectOutLabX(int num)
	{		
		float[] rect=new float[4];
		int x=num%11;
		int y=num/11;
		rect[0]=(float) (x*xSpace+1);
		rect[1]=(float) (y*ySpace+1.5);
		rect[2]=(float) ((x+1)*xSpace-1);
		rect[3]=(float) ((y+1)*ySpace-1);
		return rect;
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_testup);
		LoadXML();
		InitialAreaTest();
		LoadLastSet();
		InitialStatus();
		...
		Thread updateStatus = new Thread(new Runnable()
		{
		    ...
		    {
			// TODO Auto-generated method stub
			while (MidData.m_isInterrupted_LowerMachine&&MidData.isOnTestUp)
			{
				    ...
				    public void run()
				    {
				    	...
				    	if(MidData.mSetParameters.SetNumWay==0)
				    	{
					    	...
					    	for(int i=0;i<100;i++)
					    	{
					    		...
					    		{
					    			final int num=i;
					    			new AlertDialog.Builder(TestUpActivity.this)
						    ...
						    else if(MidData.IsNeedUserConfirm==3)
						    {
						    	...
						    	for(int i=0;i<100;i++)
						    	{
						    		...
						    		{
						    			//System.out.println(i+"---------i---"+MidData.mTestParameters[i].aisleNum);
							...
							if (MidData.IsNeedReTest == 1)
							{
							    ...
							    new AlertDialog.Builder(TestUpActivity.this)
								    .setTitle(MidData.IsChinese==1?"消息提示":"Notice")
								    .setMessage(
								    		MidData.IsChinese==1?"测试值异常 请重新测试 异常通道" + msg:"Abnormal channels:Abnormal test value,please re-testing" + msg)
								    .setPositiveButton(
								    		MidData.IsChinese==1?"确定":"Comfirm",
									    new DialogInterface.OnClickListener()
									    {
	...
	private void LoadLastSet(){
		...
		button_NextPage.setText(MidData.IsChinese==1?"下页":"Page\nDown");
	}
	private void updateStatusShow(){
		if(MidData.TestFinished)
		{		
			
			if(MidData.SelAddress>=0&&MidData.SelAddress<=50&&MidData.SelAddress!=LastTouchNum)
			{
				int ad=LastTouchNum;
			...
			for(int i=0;i<50;i++)
			{
				...
				if(updatestatus[i]==0)
				{
					if(MidData.mTestParameters[i].aisleStatus==-1)//通道不可用
					{
						st=10;
					}
					else
					{						
						if(MidData.IsESRSelect[i])
						{
							...
							else if(MidData.mTestParameters[i].testStatus==6)//耗材不足不足
							{
								findERSSample=true;
							}
						}
					...
					b.putInt("编号", i);
					msg.setData(b);
					TestUpActivity.this.mHandler.sendMessage(msg);
				...
				else
				{
					if(updatestatus[i]!=10)
					{
						if(MidData.IsESRSelect[i])
						{
							...
							else if(MidData.mTestParameters[i].testStatus==6)//耗材不足
							{
								findERSSample=true;
							}
						}
	...
	private void LoadXML(){
		...
		button_NextPage.setOnClickListener(new button_NextPageClickListener());
		button_NextPage.setOnTouchListener(new ButtonTouchListener());
		...
		TestUpArea=(ImageView)findViewById(R.id.TestUpArea);
		TestUpArea.setOnTouchListener(new TestUpAreaTouchListener());
	}
	...
	class button_ReTestClickListener implements OnClickListener{
		...
		public void onClick(View arg0) {
			...
			intent.setClass(TestUpActivity.this, ReTestSetActivity.class);
			startActivity(intent);
		}
		
	}
	class button_NextPageClickListener implements OnClickListener{

		@Override
		public void onClick(View arg0) {
			// TODO Auto-generated method stub			
			MidData.isOnTestUp = false;			
			MidData.isOnTestDown = true;
			Intent intent = new Intent();
			intent.setClass(TestUpActivity.this, TestDownActivity.class);
			startActivity(intent);
			overridePendingTransition(R.anim.enter_bottomtotop,
	...
	class TestUpAreaTouchListener implements OnTouchListener{
		...
		public boolean onTouch(View arg0, MotionEvent arg1) {
			...
			if(num%11!=5&&num<55)
			{
				...
				LastTouchNum=newNum;
				MidData.SelAddress=newNum;
				//Toast.makeText(getApplicationContext(), ""+MidData.SelAddress, Toast.LENGTH_LONG).show();
				
				UpdateSelArea(newNum);
				Intent curve=new Intent();
				Bundle b=new Bundle();
				b.putInt("通道号", LastTouchNum);
				b.putString("位置", GetAddress(LastTouchNum));
				curve.putExtras(b);
				curve.setClass(TestUpActivity.this, CurveActivity.class);
				startActivity(curve);
			}
	...
	class button_ERSTestListener implements OnClickListener{
		...
		public void onClick(View arg0) {
			...
			if (MidData.mTestParameters[k].testStatus==1)
			{
			    Toast.makeText(getApplicationContext(), MidData.IsChinese==1?"正在发现样本，请稍后设置！":"Samples are found, please set later",
				    Toast.LENGTH_LONG).show();
			    return;
			}
	...
	private void InitialAreaTest(){
		...
		for(int i=0;i<10;i++)
		{
			...
			/*if(i==0)
				...
				rect=GetRectOutLabS(num);
			else if(i==2||i==4)
				rect=GetRectOutLabX(num);*/
			canvas.drawRect(rect[0], rect[1], rect[2], rect[3], labbackPaint);
			if(i<5)
				canvas.drawText(Integer.toString(i), 30+5*xSpace, 50+i*ySpace, textLabPaint);
			else if(i>=5)
				canvas.drawText(Integer.toString(i), 30+5*xSpace, 50+(i+1)*ySpace, textLabPaint);
		}
		for(int i=0;i<10;i++)
		{
			...
			if(i<5)
			{
				float[] rect=GetRectOut(55+i);
				
				canvas.drawRect(rect[0], rect[1], rect[2], rect[3], labbackPaint);
				canvas.drawText(datatoString(i), 30+i*xSpace, 50+5*ySpace, textLabPaint);
			}
				
			else if(i>=5&&i<10)
			{
				float[] rect=GetRectOut(55+i+1);
				
				canvas.drawRect(rect[0], rect[1], rect[2], rect[3], labbackPaint);
				
				canvas.drawText(datatoString(i), 30+(i+1)*xSpace, 50+5*ySpace, textLabPaint);
			}
		...
		canvas.drawText("X", 30+5*xSpace, 50+5*ySpace, labbackPaint);
		
		/*for(int i=0;i<10;i++)
		{
			float[] rect=GetRectOut(i);
			Path path1=new Path();
			path1.moveTo(rect[0]+15, rect[1]+15);
			path1.lineTo(rect[2]-15, rect[3]-15);
			Path path2=new Path();
			path2.moveTo(rect[0]+15, rect[3]-15);
			path2.lineTo(rect[2]-15, rect[1]+15);
			canvas.drawPath(path1, picPaint);
			canvas.drawPath(path2, picPaint);	
		}*/
